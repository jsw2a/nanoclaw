...
const PRODUCTS = [
  {
    id: "pitchers", name: "Talavera Pitchers", tag: "Pitchers", price: 68,
  ...
  {
    id: "vases", name: "Bud Vases", tag: "Vases", price: 34,
  ...
  {
    id: "dishes", name: "Trinket Dishes", tag: "Dishes", price: 22,
  ...
  {
    id: "tumblers", name: "Painted Tumblers", tag: "Drinkware", price: 28,
  ...
  {
    id: "bowls", name: "Nesting Bowls", tag: "Bowls", price: 56,
...
/* ---- Immersive ware panels (alternating tile walls) ---- */
document.getElementById("wares").innerHTML = PRODUCTS.map((p, i) => `
  <section class="ware tile-wall ${i % 2 === 0 ? "cobalt" : "terracotta"}" id="${p.id}">
    <div class="ware-inner">
      <span class="ware-num" aria-hidden="true">${String(i + 1).padStart(2, "0")}</span>
      <figure class="ware-photo">
        <div class="mat">
...
function render() {
  let count = 0, total = 0;
  for (const [id, qty] of cart) { count += qty; total += PRODUCTS.find((p) => p.id === id).price * qty; }
  cartCount.textContent = count;
  cartTotal.textContent = "$" + total;
  if (cart.size === 0) { cartItems.innerHTML = '<p class="cart-empty">Your bag is empty.</p>'; return; }
